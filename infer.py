...
###########################
app = FastAPI()
conversation_history = deque(maxlen=10)  # 대화 기록 저장
...
def initialize_local_llm(model_path: str, max_seq_length: int, dtype=torch.float16, load_in_4bit=False):
    """Load and prepare the local LLM model."""
...
def generate_prompt(context: str, input_message: str, conversation_history):
    """Generate a prompt with context, input message, and conversation history."""
    meta_prompt = """Context:
{}
